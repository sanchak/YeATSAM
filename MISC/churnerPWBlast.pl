...
while (my $c = $iter->next) {
    my @combo = @{$c} ;
    my ($a,$b) = @combo ;
	my $OUT = "SCORES/$a.$b.blastpw.out";
	if(! -e $OUT){
	    system( "myblastcompare2Fastafiles.csh $FASTADIR/$a.ALL.1.fasta $FASTADIR/$b.ALL.1.fasta $OUT P");
	}
	my $expect = util_ParseBlastPW($OUT);
	print $ofh "$a $b $expect\n";
