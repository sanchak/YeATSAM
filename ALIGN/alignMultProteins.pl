...
my ($moveZ,$verify,$radii,$before1,$before2);
$readpotential = 0 ;
GetOptions(
...
my ($annMap,$NNN) = util_maketablefromfile($ann);
...
foreach my $idx (0..$N){
	 ...
	 push @done, $DD ;
	 if(defined $moveZ && $idx){
	     $PPP->MoveAtomsAlongAxis("Z",$moveZ);
	}
...
sub Match2Proteins{
	...
	my $potatom2   = 0 ;
	if($readpotential){
	$potatom1  = util_GetPotForAtom($firstatom1,$pqr1,$pots1) ;
	$potatom2  = util_GetPotForAtom($firstatom2,$pqr2,$pots2) ;
	}
    ...
    foreach my $i  (@sortedClose){
		...
		my $TYPE1 = $i->GetResName();

		next if(($TYPE1 eq "GLY" || $TYPE1 eq "G") && $POTENTIALMATCH);

		push @allresnum, $RESNUMi ; 
    
	    my @tmp1 = (@{$done1}, $i);
        my $poti = 0 ;
        $poti = util_GetPotForAtom($i,$pqr1,$pots1) if($potcongruence && $readpotential);

	    print $ofhclose "neighboring Atom\n";
		print $ofhclose " NEIGH $NNNNN \n";
		PrintMatch($MATCH,$NNNNN,$ofhclose,$pdb1,$pdb2);
	    ...
	    foreach my $j (@sorted){
			...
			## Provide some weightage to the same res type
		    my $atomstrI = $i->GetType();
		    my $atomstrJ = $j->GetType();

			my $RESNUMiPlusOne = $RESNUMi + 1;
			my $RESNUMiMinusOne = $RESNUMi - 1;

			if($TYPE1 eq $TYPE2 && $atomstrI eq $atomstrJ){
				$normalizedSum = $normalizedSum -1 ;
				print $ofhclose "Info Adding weightage as same $TYPE1 eq $TYPE2 && $atomstrI eq $atomstrJ \n";

			    if(exists $MATCH->{$RESNUMiPlusOne} && $MATCH->{$RESNUMiPlusOne} eq $RESNUMj+1){

				    print $ofhclose "Info Adding further weightage as next mapped\n";
					
					$normalizedSum = 0 ;
			    }
			    if(exists $MATCH->{$RESNUMiMinusOne} && $MATCH->{$RESNUMiMinusOne} eq $RESNUMj-1){
				    print $ofhclose "Info Adding further weightage as prev mapped\n";
					$normalizedSum = 0 ;
			    }
	    ...
	    foreach my $j (@sortedrsmd){
            ...
            my $potj = 0;
            $potj = util_GetPotForAtom($j,$pqr2,$pots2) if($potcongruence && $readpotential);
			...
			if($potcongruence){
			      if(!$added && !exists $alreadyadded->{$RESNUMj} && abs($diff) < 50){
				              print "diff = $diff \n";
...
sub Match2Indices{
		 ...
		 if($POTENTIALMATCH){
		     ...
		     PrintMatch($ORIGMATCH,"0",$ofhclose,$pdb1,$pdb2);
		 }
		...
		PrintMatch($ORIGMATCH,"1",$ofhclose,$pdb1,$pdb2);
		return $ORIGMATCH ;
}
...
sub PrintMatch{
   ...
   my $cnt = 0 ; 
   my $CNT = 0 ; 
   my $ofh1 = util_write("1.clasp.in");
   my $ofh2 = util_write("2.clasp.in");
   foreach my $i (sort {$a <=> $b} keys %{$MATCH}){
	   ...
	   push @a , $i ;
	   push @b , $j ;
	   my ($res1) = $pdb1->GetResidueIdx($i);
	   my ($type1) = $res1->PrintSingleLetter($pdb1);
	   my $NM1 = $res1->GetName();
	   my $NM2 = "" ;
	   my $type2 = "-";
	   if($j ne "-"){
	       my ($res2) = $pdb2->GetResidueIdx($j);
		   $NM2 = $res2->GetName();	
	       print "Getting index $j\n";
	       ($type2) = $res2->PrintSingleLetter($pdb2);
	   }



	   print $OFH "$str $type1$i = $type2$j\n";
	   print $ofh1 "$NM1/$i/CB ";
	   print $ofh2 "$NM2/$j/CB ";
   }
print $ofh1 " \n";
