...
sub util_PrintPymolManyProteins{
    my ($ofh,@pdbs) = @_ ; 
	my $N = @pdbs ;
	print "There are $N pdbs \n";

    #my @colors = qw ( red green blue magenta cyan yellow orange);
    #my @colors = qw ( red green red green red green blue magenta );
    #my @colors = qw ( black grey black grey black grey black grey black grey black grey black grey );
	...
	foreach my $pdb (@pdbs){
		...
		my $nm = "PROT$cnt";
		push @nms, $nm ;
        util_PrintPymolShell($ofh,$pdb,$nm);
	}
	...
	foreach my $idx (0..$NNN){
	     my $nm = $nms[$idx];
	     my $pdb = $pdbs[$idx];
         print $ofh "load_embedded $nm,name=$nm,state=1,discrete=0 \n";
	     die if(!@colors);
	     my $col = shift @colors ;
         print $ofh "color $col, $nm \n";
		 print "$pdb:$col\n";
	}
    print $ofh "hide all \n";
    print $ofh "show cartoon \n";
...
sub util_PrintPymolWith2Proteins{
    ...
    print $ofh "color grey, query \n";
    print $ofh "color wheat, target \n";
...
my @colors = qw ( cyan magenta yellow orange);
...
sub util_PrintPymolSingleProtein{
    ...
    print $ofh "load_embedded topmatch_target,name=PDBA,state=1,discrete=0  \n";
    print $ofh "set transparency = 0.9  \n";
    print $ofh "hide everything, PDBA  \n";
    print $ofh "select target, PDBA   \n";
    print $ofh "bg_color white  \n";
    print $ofh "set specular, 0  \n";
    print $ofh "color wheat, PDBA  \n";
    ...
    my $ifh = MyUtils::util_read($infile);
	my $cnt = 0; 
    while(<$ifh>){
         next if(/^\s*$/);
         next if(/^\s*#/);
	...
	print $ofh "#select active1, (chain A and resi 888 )\n";
	print $ofh "#show sticks, active1\n";
	print $ofh "#color red, active1\n";
